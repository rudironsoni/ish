...
#define IMPLEMENTED_FLAGS                                                                          \
    (CLONE_VM_ | CLONE_FILES_ | CLONE_FS_ | CLONE_SIGHAND_ | CLONE_SYSVSEM_ | CLONE_VFORK_ |       \
     CLONE_THREAD_ | CLONE_SETTLS_ | CLONE_CHILD_SETTID_ | CLONE_PARENT_SETTID_ |                  \
     CLONE_CHILD_CLEARTID_ | CLONE_DETACHED_)
                     ...
                     addr_t tls_addr, addr_t ctid_addr)
{
    ...
    int err;
    // CRITICAL: Use current->mm (parent's mm) not task->mm (child's mm)
    // Child's mm is NULL after explicit initialization refactor
    struct mm *parent_mm = current->mm;
    if (parent_mm == NULL) {
        die("copy_task: current->mm is NULL");
    }
