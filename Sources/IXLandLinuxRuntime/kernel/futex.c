...
static int futex_load(struct futex *futex, uint32_t *out)
{
    assert(futex->mem == current->mem);
    read_wrlock(&current->mem->lock);
    uint32_t *ptr = mem_ptr(current->mem, futex->addr, MEM_READ);
...
static int futex_wait(addr_t uaddr, uint32_t val, struct timespec *timeout)
{
...
static int futex_wakelike(int op, addr_t uaddr, uint32_t wake_max, uint32_t requeue_max,
                          addr_t requeue_addr)
{
    struct futex *futex = futex_get(uaddr);
...
int futex_wake(addr_t uaddr, uint32_t wake_max)
{
