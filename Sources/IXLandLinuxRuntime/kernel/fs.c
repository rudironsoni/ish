...
uint32_t sys_faccessat(fd_t at_f, addr_t path_addr, mode_t_ mode, uint32_t flags)
{
    char path[MAX_PATH];
...
fd_t sys_openat(fd_t at_f, addr_t path_addr, uint32_t flags, mode_t_ mode)
{
    char path[MAX_PATH];
    if (user_read_string(path_addr, path, sizeof(path)))
        return _EFAULT;
...
uint32_t sys_readlinkat(fd_t at_f, addr_t path_addr, addr_t buf_addr, uint32_t bufsize)
{
    char path[MAX_PATH];
    if (user_read_string(path_addr, path, sizeof(path)))
        return _EFAULT;
...
uint32_t sys_read(fd_t fd_no, addr_t buf_addr, uint32_t size)
{
    ...
    if (buf == NULL)
        return _ENOMEM;
    int64_t res = sys_read_buf(fd_no, buf, size);
    if (res >= 0) {
...
{
    uint32_t iovec_size = sizeof(struct iovec_) * iovec_count;
...
uint32_t sys__llseek(fd_t f, uint32_t off_high, uint32_t off_low, addr_t res_addr, uint32_t whence)
{
    ...
    if (!fd->ops->lseek)
        return _ESPIPE;
    lock(&fd->lock);
    off_t_ off = ((uint64_t)off_high << 32) | off_low;
...
static int64_t statfs_mount(struct mount *mount, addr_t buf_addr)
{
...
static int64_t statfs64_mount(struct mount *mount, addr_t buf_addr)
{
    struct statfsbuf buf = {};
    int err = mount_statfs(mount, &buf);
    if (err < 0)
...
static uint32_t sys_utime_common(fd_t at_f, addr_t path_addr, struct timespec atime,
                                 struct timespec mtime, uint32_t flags)
{
    char path[MAX_PATH];
    if (path_addr != 0)
        if (user_read_string(path_addr, path, sizeof(path)))
...
uint32_t sys_fchown32(fd_t f, uid_t_ owner, uid_t_ group)
{
...
{
    off_t_ size = ((uint64_t)size_high << 32) | size_low;
    char path[MAX_PATH];
    if (user_read_string(path_addr, path, sizeof(path)))
        return _EFAULT;
...
uint32_t sys_fallocate(fd_t f, uint32_t UNUSED(mode), uint32_t offset_low, uint32_t offset_high,
                       uint32_t len_low, uint32_t len_high)
{
    off_t_ offset = ((uint64_t)offset_high << 32) | offset_low;
    off_t_ len = ((uint64_t)len_high << 32) | len_low;
    struct fd *fd = f_get(f);
    if (fd == NULL)
        return _EBADF;
...
// a few stubs
uint32_t sys_sendfile(fd_t UNUSED(out_fd), fd_t UNUSED(in_fd), addr_t UNUSED(offset_addr),
                      uint32_t UNUSED(count))
{
    return _EINVAL;
}
uint32_t sys_sendfile64(fd_t UNUSED(out_fd), fd_t UNUSED(in_fd), addr_t UNUSED(offset_addr),
                        uint32_t UNUSED(count))
{
    return _EINVAL;
}
uint32_t sys_splice(fd_t UNUSED(in_fd), addr_t UNUSED(in_off_addr), fd_t UNUSED(out_fd),
                    addr_t UNUSED(out_off_addr), uint32_t UNUSED(count), uint32_t UNUSED(flags))
{
    return _EINVAL;
}
uint32_t sys_copy_file_range(fd_t UNUSED(in_fd), addr_t UNUSED(in_off), fd_t UNUSED(out_fd),
                             addr_t UNUSED(out_off), uint32_t UNUSED(len), uint64_t UNUSED(flags))
{
    return _EPERM; // good enough for ruby
}

uint32_t sys_xattr_stub(addr_t UNUSED(path_addr), addr_t UNUSED(name_addr),
                        addr_t UNUSED(value_addr), uint32_t UNUSED(size), uint32_t UNUSED(flags))
{
    return _ENOTSUP;
}
