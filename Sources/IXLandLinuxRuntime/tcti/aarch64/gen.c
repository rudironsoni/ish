...
static int emit_logical_reg_fallback(a64_gen_state_t *state, int rd, int rn, int rm,
                                     int shift_type, int imm_shift, int subtype, int set_flags,
                                     int is_64bit)
{
...
int a64_gen_dp_imm(a64_gen_state_t *state, const a64_instr_t *instr)
{
    ...
    case 0: // ADR/ADRP (op0=000) or MOVN (opc=00 in op0=010)
        ...
        if (instr->cat == A64_SIMD0) {
            ...
            uint64_t value = instr->is_64bit ? ~((uint64_t)instr->imm)
                                             : (uint32_t)~((uint32_t)instr->imm);
...
int a64_gen_dp_reg(a64_gen_state_t *state, const a64_instr_t *instr)
{
    ...
    if (op2 >= 0 && op2 <= 3) {
        ...
        if (instr->imm_shift != 0 &&
            (src1_is_memory || src2_is_memory || dst_is_memory ||
        ...
        ret = emit_addsub_reg_fallback(state, eff_rd_for_op, eff_rn_for_op, 13, A64_SHIFT_LSL,
                                       instr->imm_shift, instr->subtype == 1, instr->set_flags,
                                       instr->is_64bit);
        if (ret != A64_GEN_OK)
            return ret;
