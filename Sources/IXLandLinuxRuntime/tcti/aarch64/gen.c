...
static int a64_gen_simd(a64_gen_state_t *state, const a64_instr_t *instr)
{
    ...
    case A64_SIMD_XTN2:
        ret = emit_gadget(state, gadget_simd_xtn2);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_ZIP2:
        ret = emit_gadget(state, gadget_simd_zip2);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_TRN2:
        ret = emit_gadget(state, gadget_simd_trn2);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_UZP2:
        ret = emit_gadget(state, gadget_simd_uzp2);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_BIC:
        ret = emit_gadget(state, gadget_simd_bic);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_SUB:
        ret = emit_gadget(state, gadget_simd_sub);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_MUL:
        ret = emit_gadget(state, gadget_simd_mul);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_ORN:
        ret = emit_gadget(state, gadget_simd_orn);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_BSL:
        ret = emit_gadget(state, gadget_simd_bsl);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_CMGE:
        ret = emit_gadget(state, gadget_simd_cmge);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_CMHI:
        ret = emit_gadget(state, gadget_simd_cmhi);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_CMHS:
        ret = emit_gadget(state, gadget_simd_cmhs);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_CMLE:
        ret = emit_gadget(state, gadget_simd_cmle);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_CMLT:
        ret = emit_gadget(state, gadget_simd_cmlt);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
    ...
    case A64_SIMD_CMTST:
        ret = emit_gadget(state, gadget_simd_cmtst);
        if (ret != A64_GEN_OK)
            return ret;
        ret = emit_u64(state, instr->Rd);
        if (ret != A64_GEN_OK)
