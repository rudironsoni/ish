...
// Reset global fiber context for test harness isolation
...
static inline void fiber_report_tlb_stats(struct fiber_exec_ctx *ctx)
{
    ...
    uint64_t reads =
        fiber_stat_get(ctx, STAT_TLB_READ_HITS) + fiber_stat_get(ctx, STAT_TLB_READ_MISSES);
    uint64_t writes =
        fiber_stat_get(ctx, STAT_TLB_WRITE_HITS) + fiber_stat_get(ctx, STAT_TLB_WRITE_MISSES);
