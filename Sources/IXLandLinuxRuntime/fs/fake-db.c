...
static void db_check_error(struct fakefs_db *fs)
{
    ...
    default:
        die("sqlite error: %d %#x %s", errcode, sqlite3_extended_errcode(fs->db),
...
static sqlite3_stmt *db_prepare(struct fakefs_db *fs, const char *stmt)
{
    ...
    sqlite3_prepare_v2(fs->db, stmt, (int)(strlen(stmt) + 1), &statement, NULL);
    db_check_error(fs);
    return statement;
}
...
{
    sqlite3_bind_blob(stmt, i, path, (int)strlen(path), SQLITE_TRANSIENT);
}
...
void path_rename(struct fakefs_db *fs, const char *src, const char *dst)
{
    ...
    src_extra[src_len] = '/';
    sqlite3_bind_blob(fs->stmt.path_rename, 3, src_extra, (int)(src_len + 1), SQLITE_TRANSIENT);
    src_extra[src_len] = '0';
    sqlite3_bind_blob(fs->stmt.path_rename, 4, src_extra, (int)(src_len + 1), SQLITE_TRANSIENT);
    sqlite3_bind_blob(fs->stmt.path_rename, 5, src_extra, (int)src_len, SQLITE_TRANSIENT);
...
{
    char *sql = _sql;
    printk("%d sql trace: %s %s\n", current ? current->pid : -1, sqlite3_expanded_sql(stmt),
           sql[0] == '-' ? sql : "");
    return 0;
}
#endif
...
int fake_db_init(struct fakefs_db *fs, const char *db_path, int root_fd)
{
    ...
    statement = db_prepare(
        fs, "delete from stats where not exists (select 1 from paths where inode = stats.inode)");
    db_check_error(fs);
    sqlite3_step(statement);
    db_check_error(fs);
    sqlite3_finalize(statement);
