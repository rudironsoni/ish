...
static int proc_show_stat(struct proc_entry *UNUSED(entry), struct proc_data *buf)
{
    ...
    proc_printf(buf, "cpu  %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 "\n", usage.user_ticks,
                usage.nice_ticks, usage.system_ticks, usage.idle_ticks);
    ...
    struct timespec uptime_ts = { .tv_sec = uptime.uptime_ticks / 100,
                                  .tv_nsec = uptime.uptime_ticks % 100 };
...
#define proc_printf_comma(buf, at_start, format, ...)                                              \
    do {                                                                                           \
...
static bool proc_root_readdir(struct proc_entry *UNUSED(entry), unsigned long *index,
