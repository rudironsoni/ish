...
static int write_tooling_log(const char *artifact_dir, const char *case_id,
...
static int test_tool_001_gcc_clang_guest_build(const char *artifact_dir, char *log_buf, size_t log_size) {
    (void)artifact_dir;
    printf("TOOL-001: Testing guest compiler availability...\n");

    int log_pos = 0;
    log_pos += snprintf(log_buf + log_pos, log_size - log_pos,
                        "    {\"event\": \"compiler_check\"},\n");
...
static int test_stab_001_shell_loop_stability(const char *artifact_dir, char *log_buf, size_t log_size) {
    (void)artifact_dir;
    printf("STAB-001: Testing shell loop stability...\n");

    int log_pos = 0;
    log_pos += snprintf(log_buf + log_pos, log_size - log_pos,
                        "    {\"event\": \"shell_loop_start\"},\n");
...
static int test_stab_003_thread_storm(const char *artifact_dir, char *log_buf, size_t log_size) {
    ...
    struct stab003_thread_data *thread_data = calloc(num_threads, sizeof(struct stab003_thread_data));
...
static int test_ios_001_execution_strategy_sandbox(const char *artifact_dir, char *log_buf, size_t log_size) {
    (void)artifact_dir;
...
static int test_ios_003_trace_artifact_export(const char *artifact_dir, char *log_buf, size_t log_size) {
    (void)artifact_dir;
...
int main(int argc, char *argv[]) {
    ...
    /* Write report */
    if (write_report(artifact_dir, case_id, "10-tooling-stability-ios", "ios_fixture",
