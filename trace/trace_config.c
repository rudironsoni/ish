...
static int parse_pc_range(const char *range_str, uint64_t *start, uint64_t *end) {
    ...
    *dash = '\0';
    ...
    /* Trim whitespace */
    while (*start_str == ' ' || *start_str == '\t') start_str++;
...
int trace_config_from_env(trace_config_t *config) {
    ...
    if (dump_on) {
        if (strcmp(dump_on, "fault") == 0 || 
            strcmp(dump_on, "all") == 0) {
...
void trace_config_print(trace_config_t *config) {
    ...
    switch (config->backend) {
        case TRACE_BACKEND_NOP: fprintf(stderr, "nop\n"); break;
        case TRACE_BACKEND_RING: fprintf(stderr, "ring\n"); break;
        case TRACE_BACKEND_STDERR: fprintf(stderr, "stderr\n"); break;
