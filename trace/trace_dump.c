    ...
    if (sidecar->insn_count > 0) {
        ...
        for (uint32_t i = 0; i < sidecar->insn_count; i++) {
            ...
            if (insn->num_dsts > 0) {
                ...
                for (int j = 0; j < insn->num_dsts && pos < sizeof(reg_str) - 4; j++) {
            ...
            if (insn->num_srcs > 0) {
                if (insn->num_dsts > 0) pos += snprintf(reg_str + pos, sizeof(reg_str) - pos, " ");
                pos += snprintf(reg_str + pos, sizeof(reg_str) - pos, "S:");
                for (int j = 0; j < insn->num_srcs && pos < sizeof(reg_str) - 4; j++) {
            ...
            fprintf(fp, "%-4u 0x%016llx 0x%08x   %-32s %-20s\n",
                    i,
                    (unsigned long long)insn->pc,
                    insn->raw_insn,
                    insn->mnemonic[0] ? insn->mnemonic : "(unknown)",
                    reg_str);
        }
    }
