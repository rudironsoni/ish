...
- If a rule matters more than once, it should be encoded in `the control plane documentation`.
- `AGENTS` should be a navigable map, not an unstructured encyclopedia.

## Required behavior

- Update `the control plane documentation` when a repeated rule, pattern, or failure mode becomes important.
