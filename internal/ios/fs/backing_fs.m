...
static bool mount_from_bookmarks = false;
...
static const char *path_for_url_in_mount(struct mount *mount, NSURL *url, const char *fallback) {
    NSString *mount_path = url_for_mount(mount).path;
    NSString *url_path = url.path;

    if ([mount_path hasPrefix:@"/private/"])
        mount_path = [mount_path substringFromIndex:8];
    if ([url_path hasPrefix:@"/private/"])
        url_path = [url_path substringFromIndex:8];
...
static int posix_error_from_nserror(NSError *error) {
    if (error == nil)
        return 0;
    while (error != nil) {
...
static int combine_error(NSError *coordinatorError, int err) {
    int posix_error = posix_error_from_nserror(coordinatorError);
...
static struct fd *iosfs_open(struct mount *mount, const char *path, int flags, int mode) {
    ...
    if (err == 0 && S_ISREG(stats.mode)) {
        ...
        int posix_error = posix_error_from_nserror(error);
...
static int iosfs_fstat(struct fd *fd, struct statbuf *fake_stat) {
    return realfs.fstat(fd, fake_stat);
}
