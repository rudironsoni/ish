...
// POSIX timers for continuous profiling (Linux only)
#if defined(__linux__)
#include <time.h>
#define HAS_POSIX_TIMERS 1
#else
#define HAS_POSIX_TIMERS 0
#endif
...
static void buffer_write(const char *data, size_t len) {
    if (!g_prof_writer.enabled || g_prof_writer.fd < 0) return;
...
static void buffer_json_string(const char *str) {
    ...
    for (const char *p = str; *p; p++) {
        switch (*p) {
            case '"': buffer_write("\\\"", 2); break;
            case '\\': buffer_write("\\\\", 2); break;
            case '\b': buffer_write("\\b", 2); break;
            case '\f': buffer_write("\\f", 2); break;
            case '\n': buffer_write("\\n", 2); break;
            case '\r': buffer_write("\\r", 2); break;
            case '\t': buffer_write("\\t", 2); break;
            default:
                if (*p >= 0x20 && *p < 0x7f) {
                    buffer_write(p, 1);
...
void prof_shutdown(void) {
    if (!g_prof_writer.enabled) return;
...
void prof_end(uint64_t handle, prof_event_type_t type) {
    if (!g_prof_writer.enabled || handle == 0) return;
...
uint64_t prof_get_duration_ns(void) {
    if (!g_prof_writer.enabled) return 0;
...
bool prof_write_flamegraph(const char *output_path) {
    if (!g_prof_writer.enabled) return false;
...
#if HAS_POSIX_TIMERS
static void continuous_handler(union sigval val) {
    (void)val;
    if (g_prof_writer.enabled) {
        prof_capture_sample(PROF_EVENT_SAMPLE);
        flush_buffer();
    }
}
#endif
...
bool prof_start_continuous(uint32_t interval_ms) {
    ...
    struct sigevent sev = {
        ...
        .sigev_notify_function = continuous_handler,
        .sigev_notify_attributes = NULL
    };
    
    if (timer_create(CLOCK_MONOTONIC, &sev, &g_continuous.timer) != 0) {
        pthread_mutex_unlock(&g_continuous.lock);
        return false;
    }
    ...
    struct itimerspec its = {
        ...
        .it_value.tv_nsec = (interval_ms % 1000) * 1000000,
        .it_interval.tv_sec = interval_ms / 1000,
        .it_interval.tv_nsec = (interval_ms % 1000) * 1000000
    };
    
    if (timer_settime(g_continuous.timer, 0, &its, NULL) != 0) {
        timer_delete(g_continuous.timer);
        pthread_mutex_unlock(&g_continuous.lock);
        return false;
    }
    
    g_continuous.interval_ms = interval_ms;
    g_continuous.running = true;
...
static void prof_auto_init(void) {
...
static void prof_auto_shutdown(void) {
    if (g_prof_writer.enabled) {
        prof_shutdown();
    }
}
