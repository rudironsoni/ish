...
int tcti_harness_case_add_7_13_14(tcti_harness_snapshot_t *snapshot)
{
    ...
    snapshot->in_regs[14] = lhs;
    snapshot->in_regs[15] = rhs;

    tcti_harness_run_snapshot(gadget_add_reg[7][13][14], snapshot->in_regs, snapshot->out_regs);

    return snapshot->out_regs[8] == (lhs + rhs) && snapshot->out_regs[14] == lhs &&
           snapshot->out_regs[15] == rhs && snapshot->out_regs[0] == 0x1111111111111111ULL;
}

int tcti_harness_case_add_0_1_2(tcti_harness_snapshot_t *snapshot)
{
    tcti_harness_seed_inputs(snapshot->in_regs);
    snapshot->in_regs[2] = 0x40ULL;
    snapshot->in_regs[3] = 0x2ULL;
    snapshot->in_regs[8] = 0x3333333333333333ULL;

    tcti_harness_run_snapshot(gadget_add_reg[0][1][2], snapshot->in_regs, snapshot->out_regs);

    return snapshot->out_regs[1] == 0x42ULL && snapshot->out_regs[2] == 0x40ULL &&
           snapshot->out_regs[3] == 0x2ULL && snapshot->out_regs[8] == 0x3333333333333333ULL;
}

int tcti_harness_case_mov_2_7(tcti_harness_snapshot_t *snapshot)
{
    tcti_harness_seed_inputs(snapshot->in_regs);
    snapshot->in_regs[8] = 0x123456789ABCDEF0ULL;
    snapshot->in_regs[0] = 0x4444444444444444ULL;
    snapshot->in_regs[3] = 0;

    tcti_harness_run_snapshot(gadget_mov_reg[2][7], snapshot->in_regs, snapshot->out_regs);

    return snapshot->out_regs[3] == 0x123456789ABCDEF0ULL &&
           snapshot->out_regs[8] == 0x123456789ABCDEF0ULL &&
