...
static int write_decoded_json(const char *artifact_dir, const char *case_id, test_vector_t *vectors,
...
int run_decode_golden_case(const char *case_id, const char *artifact_dir)
{
    ...
    /* Write decoded.json */
    if (write_decoded_json(artifact_dir, case_id, vectors, config.num_vectors, decoded,
                           passed_flags) != 0) {
        passed = 0;
...
static int main(int argc, char *argv[])
{
    ...
    return run_decode_golden(case_yaml, artifact_dir);
}
