...
/* Forward declaration for TCTI entry call */
struct cpu_state;
...
static int map_test_page(struct cpu_state *cpu, uint64_t guest_addr, int writable)
{
    ...
    cpu->tlb->entries[tlb_idx].page_if_writable = writable ? page_base : TLB_PAGE_EMPTY;
    cpu->tlb->entries[tlb_idx].data_minus_addr = (uintptr_t)test_memory - test_memory_base;
    return 0;
}
...
int run_semantic_micro(const char *case_yaml, const char *artifact_dir)
{
    ...
    if (map_test_page(&cpu, cpu.pc, 0) != 0 || seed_test_instruction(cpu.pc, insn_word) != 0) {
        failure_summary = "failed to map instruction page for semantic execution";
        goto cleanup;
    }
    ...
    if (instr.cat == A64_LD_ST && map_test_page(&cpu, cpu.x[instr.Rn], 1) != 0) {
        failure_summary = "failed to map data page for semantic execution";
        goto cleanup;
    }
