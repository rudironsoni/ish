...
- **System of record:** `AGENTS`, `the synchronized rule set`, `tests/cases/*`, and `tests/cases/execution-order.yaml`
- **Important directories:**
  - `the ` – specialized sub-agent instructions
  - `the ` – lifecycle checkpoints
  - `the ` – mandatory operating rules
  - `the ` – repo-local agent command instructions
...
- **Ask first:** only if a genuinely ambiguous repo-wide policy conflict cannot be resolved from `the control plane documentation` and `tests/cases/`
